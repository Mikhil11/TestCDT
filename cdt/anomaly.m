function [hlin,htop,hbot] = anomaly(x,y,varargin) 
...
% 
% anomaly(...,'top',ColorSpec) specifies the top color shading, which can be described
% by RGB values or any of the Matlab short-hand color names (e.g., 'r' or 'red'). 
% 
% anomaly(...,'bottom',ColorSpec) specifies the bottom shading color. 
% 
...
% 
% See also: boundedline, area, patch, and fill. 
...
if nargin>2
   ...
   % Check for thresh (reference or relative) value declaration: 
   tmp = strncmpi(varargin,'threshold',3); 
   if any(tmp) 
      thresh = varargin{find(tmp)+1}; 
      assert(numel(thresh)<=2,'Input error: thresh value must be a scalar.') 
      tmp(find(tmp)+1)=1; 
      varargin = varargin(~tmp); 
   end
   
   % Top face color: 
   tmp = strncmpi(varargin,'topcolor',3); 
   if any(tmp) 
...
% Find zero crossings so shading will meet the threshline properly: 
% First for the bottom:
[xcb,ycb] = intersections(x,y,[x(1) x(end)],[thresh(1) thresh(1)]); % intersections is a subfunction by Douglas Schwarz, included below.

% Now for the top: 
[xct,yct] = intersections(x,y,[x(1) x(end)],[thresh(2) thresh(2)]); % intersections is a subfunction by Douglas Schwarz, included below.
...
yb(yb>thresh(1)) = thresh(1); 
yt(yt<thresh(2)) = thresh(2); 
...
% Plot the top half: 
%htop = area(xt,ytop,thresh(2),'facecolor',topcolor,varargin{:}); 
htop = patch([xt(1);xt;xt(end)],[thresh(2);yt;thresh(2)],'b','FaceColor',topcolor,'EdgeColor','none'); 

% Plot the bottom half: 
% hbot = area(xb,ybot,thresh(1),'facecolor',bottomcolor,varargin{:}); 
hbot = fill([xb(1);xb;xb(end)],[thresh(1);yb;thresh(1)],'b','FaceColor',bottomcolor,'EdgeColor','none'); 

% Plot the line (the "archive" values are just the unmanipulated values the user entered)
hlin = plot(x_archive,y_archive,'k',varargin{:}); 
